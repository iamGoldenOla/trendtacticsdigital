...
def create_top_bar_html():
    return """<!-- Top Bar -->
    <div class="top-bar-redesigned" style="overflow: visible !important; z-index: 9999 !important;">
        <div class="top-bar-container" style="justify-content: space-between; overflow: visible !important;">
            ...
            <div class="top-bar-left" style="flex: 1; display: flex; align-items: center; gap: 10px; max-width: 50%;">
                ...
                <span id="typewriter-text" class="typewriter-text" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%;">Welcome to Trendtactics Digital</span>
            </div>
...
def patch_top_bar(content):
    pattern = r'<!-- Top Bar -->\s*<div class="top-bar-redesigned">.*?</div>\s*</div>'
    if re.search(pattern, content, flags=re.DOTALL):
        return re.sub(pattern, create_top_bar_html(), content, flags=re.DOTALL)
    
    # Fallback if the first pattern fails (e.g. nested differently)
    pattern2 = r'<!-- Top Bar -->\s*<div class="top-bar-redesigned">.*?</div>\s*</div>\s*</div>'
...
def remove_main_nav_get_started(content):
    # Removes the old Get Started button from the main navigation across formatting styles.
    pattern = r'<li[^>]*>\s*<a[^>]*href=["\'][^"\']*(?:hub\.html|contact\.html)["\'][^>]*btn(?:-primary)?(?:-outline)?[^>]*>Get Started</a>\s*</li>'
...
def main():
    ...
    for html_file in html_files:
        ...
        # 2. Add Get Started hover CSS
        if "</style>" in content and ".top-get-started:hover" not in content:
            extra_css = """

...
// Compact FAQ Toggle Function
window.toggleSlimFAQ = function (element) {
    const parent = element.closest('.faq-item-slim');
    const isActive = parent.classList.contains('active');

    // Close all other slim FAQ items in the same grid
    const grid = parent.parentElement;
    grid.querySelectorAll('.faq-item-slim').forEach(item => {
        item.classList.remove('active');
    });

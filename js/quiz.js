(function () {
  ...
  async function sendResults() {
    ...
    const summary = computeResult();
    const payload = {
      score_pct: Math.round((summary.total / (sections.reduce((a, b) => a + b.questions.reduce((x, y) => x + (Math.max(...y.options.map(o => o.value))), 0), 0))) * 100),
      summary: summary,
      answers: answers,
      email: email,
    ...
    try {
      if (window.supabaseUtils && window.supabaseUtils.supabase) {
        ...
        alert('Thanks — quiz results submitted successfully!');
      } else {
      ...
      if (sendBtn) {
        ...
        sendBtn.textContent = sendBtn.dataset.originalText || 'Send Results';
      }
    }
  }
